...
public class ZooKeeperRegistry implements RegistryService {
	...
	public int registerModule(ModuleInstanceInfo moduleInfo) {
		String path = prefix + moduleInfo.getModuleId()
				+ Constants.PATH_SEPARATOR + moduleInfo.getVersion()
				+ Constants.PATH_SEPARATOR + moduleInfo.getIpAddress();
	...
	public int unRegisterModule(short moduleId, String version, String ipAddress) {
		String path = prefix + moduleId + Constants.PATH_SEPARATOR + version
				+ Constants.PATH_SEPARATOR + ipAddress;
		zkClient.delete(path);
		return 0;
	}

	// Currently the subscirbe will be thread-safe, only called by the main
	// thread. but it is OK to use AtomicReference to make sure
	...
	public void subscribe(ModuleInfo module, final RegistryNotify notify) {
		...
		String path = prefix + module.getModuleId() + Constants.PATH_SEPARATOR
				+ module.getVersion();
			...
			public void childChanged(String path, List<String> children) {
				...
				for (String childPath : children) {
					byte[] data = zkClient.getData(path
							+ Constants.PATH_SEPARATOR + childPath);
					ModuleInstanceInfo m = (ModuleInstanceInfo) SerializationUtils
							.deserialize(data);
					instanceList.add(m);
				}
	...
	private List<ModuleInstanceInfo> getModuleInstanceList(short moduleId,
			String version) {
		String path = prefix + moduleId + Constants.PATH_SEPARATOR + version;
		...
		if (childpaths != null) {
			for (String childpath : childpaths) {
				byte[] data = zkClient.getData(path + Constants.PATH_SEPARATOR
						+ childpath, new AbstractChildListener() {
					@Override
				...
				if (data != null) {
					...
					logger.warn("No Data for path: {}", path
							+ Constants.PATH_SEPARATOR + childpath);
				}
	...
	private List<ModuleInstanceInfo> getModuleInstanceList(
			List<ModuleInfo> destModuleInfoList) {
		List<ModuleInstanceInfo> moduleList = new ArrayList<ModuleInstanceInfo>();
		for (ModuleInfo m : destModuleInfoList) {
			List<ModuleInstanceInfo> list = getModuleInstanceList(
					m.getModuleId(), m.getVersion());
