...
public class ProxyFactory {
	...
	public void init(){
		String filterString  = ConfigUtils.getProperty(Constants.RPC_CLIENT_FILTER, "default");
	...
	private class DefaultInvocationHandler implements InvocationHandler{
		...
		public Object invoke(Object proxy, Method method, Object[] args)
			...
			RpcInvocation inv = new RpcInvocation(method.getDeclaringClass().getCanonicalName(),method.getName(),args);

			EventService annotation =method.getDeclaringClass().getAnnotation(EventService.class);
			
			if(annotation!=null){
				inv.setModuleId(annotation.moduleId());
			}
			else{
				inv.setModuleId((short)100);//TODO: set a configurable default dest moduleId
			}
