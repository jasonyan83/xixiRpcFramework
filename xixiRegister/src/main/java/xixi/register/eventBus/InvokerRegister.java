...
public final class InvokerRegister {
	...
	private void registerServiceMethod(Object instance, Method method) {
		...
		invokerBus.registerObserver(
				instance.getClass().getInterfaces()[0].getCanonicalName() + "-"
						+ method.getName(), serviceWrapper);
		logger.info("Registering EventTemplate,event is {}", instance
				.getClass().getSimpleName() + "-" + method.getName());

		Class<?>[] clzs = method.getParameterTypes();

		if (method.getReturnType() != void.class) {
			ArgumentTypeRepository.addArgumentType(method.getReturnType()
					.getCanonicalName(), method.getReturnType());
			if(List.class.equals(method.getReturnType())){
				ParameterizedType paramType = (ParameterizedType) method
						.getGenericReturnType();
				Type[] actualClzType = paramType.getActualTypeArguments();
				for (Type type : actualClzType) {
					ArgumentTypeRepository.addArgumentType(method.getReturnType()
							.getCanonicalName() + "-" + type.getClass(),
							type.getClass());
		...
		for (Class<?> clz : clzs) {
			if (clz != void.class) {
				ArgumentTypeRepository.addArgumentType(instance.getClass()
						.getSimpleName() + "-" + method.getName(), clz);
			}
		}
	}
	...
	private Invoker buildFilterChain(Invoker service, Method method) {
		...
		if (annotation != null && !annotation.filter().equals("")) {
			...
			for (String filterName : filterNames) {
				final Filter filter = (Filter) BeanFactoryUtil
						.getBean(filterName);
				if (filter != null) {
